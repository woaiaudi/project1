...
- (void)setTabContent:(UIControl *)tab withTabItem:(RKTabItem *)tabItem {
    ...
    //----自己添加的 徽标实现
    if (tabItem.badgeString.length!=0) {
        JSBadgeView *badgeView = [[JSBadgeView alloc] initWithParentView:interfaceElement alignment:JSBadgeViewAlignmentTopRight];
        badgeView.badgeText = tabItem.badgeString;
    }
    //Subviews frames
    if (titleLabel) {
